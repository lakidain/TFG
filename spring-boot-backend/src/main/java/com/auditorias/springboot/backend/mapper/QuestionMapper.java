...
public interface QuestionMapper {
	...
	@Select("select * from asi_audit_threat_vulnerabilities_questions "
			+ "where id_audit_threat=#{threatVulnerability} " + "and id_audit_vulnerability=#{existingVulnerability} "
			+ "and id_audit_question=#{existingQuestion} ")
	List<Audit_Threat_Vulnerability_Question> checkAssociationVulnerabilityQuestion(Long threatVulnerability,
			Long existingVulnerability, Long existingQuestion);
	 ...
	 */
	@Delete("delete from asi_audit_threat_vulnerabilities_questions " + "where id_audit_threat=#{id_audit_threat} "
			+ "and id_audit_vulnerability=#{id_audit_vulnerability} " + "and id_audit_question=#{id_audit_question}")
